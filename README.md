...
- **协作断线重连与事件增量同步（规则 2.10.0）**：动作落库时 payload 追加
  **录制帧**（与在线 `/act` 响应同源的结算事件序列）与提交后 `rev`——动作日志
  本身即逐帧数据源，增量同步无需服务端重新推演。新接口
  `GET /api/coop/teams/{id}/sync` 按**服务端游标**（章节 run 动作日志 seq /
  队伍时间线 seq / 远征事件 seq 三元组 + rev/章节锚点）下发游标之后的增量：
  动作事件（录制帧/操作者/校验点）、队伍时间线、远征事件；权限与写动作同源
  （非本队成员 403、零副作用）。章节推进（run 切换）、游标超前、落后超过
  `SYNC_ACTION_LIMIT=64` 条、或增量中含无录制帧的旧日志/损坏行时回 `reset`
  并附全量视口——前端整体对齐而非局部重放。前端同步器轮询驱动：队友动作
  在战斗中经 Phaser **逐帧补播**（与本地操作同一套动画队列），播完应用权威
  视口一次性对齐；本地操作进行中暂停本轮同步，补播期间操作锁定；自己的
  动作按 `seq` 推进游标不重复补播。**冲突恢复**：`expected_rev` 过期 409 时
  前端先走增量同步追平（而非全量刷新），再用最新 rev 重试，`request_id`
  幂等保证同一意图只生效一次。**断线重连**：全量入口
  `GET /api/coop/teams/{id}/expedition` 响应携带权威游标，重连后直接落在
  最新状态再增量跟随，无需逐帧补播历史。回放新增 `frame` 校验维度——
  推演帧与录制帧逐条比对（`verification.frame_mismatch`），与状态校验点
  共同背书「结果一致 + 过程逐帧一致」；录制帧不参与校验点哈希，2.10.0
  之前的旧日志无该字段，回放跳过帧比对、增量同步按 `replay_only` 回退全量。
...
帧逐条相等且 frame 维度全 ok、旧日志剥帧后回放跳过帧比对且同步回退全量、首次连接/
章节推进/游标超前/落后过多一律 reset 全量对齐、过期 expected_rev 409 后增量追平再
重试成功且 request_id 幂等只生效一次、战败结算后同步正常下发 settle 事件与终态视口、
全量入口携带权威游标）**。
...
- `GET  /api/coop/teams/{id}/sync?member_id=&run_id=&run_seq=&team_seq=&exp_seq=`
  ...
  无录制帧）、`team_events`、`expedition_events`，以及最新权威 `cursor`；
